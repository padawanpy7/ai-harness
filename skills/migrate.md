...
- **Un AGENTS.md raíz** en `/opt/` que es un **índice**: reglas de la infra compartida (redes,
  "nada publica puertos salvo nginx", cómo se despliega cada uno) + links a cada harness. (El
  `/opt/CLAUDE.md` actual ya es eso: mígralo a ese índice.)
...
2. **Setup de tools + indexar**: copia `init.sh` al proyecto y córrelo (`./init.sh`) - instala
...
3. **Adopt** (skill `adopt`): llena AGENTS.md/project.yml/playbooks/specs desde el código real.
...
5. **Limpiar comentarios**: `scripts/strip-comments.sh --write <dir>` (AST, no regex):
   ...
   que usar el AST** (createSourceFile + comment ranges + JsxExpression vacíos `{/* */}`). En
...
6. **Checks**: `check.sh` (lint/format/secretos/audit). `check-dep.sh` por paquete deprecado o
   vulnerable -> anótalo, no lo cambies ahora.
7. **Verify**: build + e2e en navegador. Debe comportarse **igual** que el baseline. Si difiere,
   rollback.
8. **Commit del proyecto, aislado.** Recién ahí pasas al siguiente.
