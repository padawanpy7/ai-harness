...
if [ -f scripts/calidad/spell.sh ] && [ -f cspell.json ]; then
  echo "==> ortografia es,en"
  issues="$(bash scripts/calidad/spell.sh 2>/dev/null | grep -c 'Unknown word' || true)"
  if [ "${issues:-0}" -gt 0 ]; then
    ...
    echo "   Verlas: scripts/calidad/spell.sh"
    fail=1
  else echo "   OK"; fi
fi
