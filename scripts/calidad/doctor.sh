...
if [ -f scripts/harness/skill-sync.sh ]; then
  before="$(cat skills/REGISTRY.md 2>/dev/null || true)"
  bash scripts/harness/skill-sync.sh >/dev/null 2>&1 || true
...
[ -f scripts/harness/features.sh ] || { echo "  ! falta scripts/harness/features.sh (ledger del build)"; warn=1; }
